...
select * from trophee t  where t.date_prise between '2052-05-01' and '2052-06-30';
...
join village v on h.num_village = v.num_village where v.nom_village = 'Aquilona';
...
select r.nom_resserre, r.superficie , v.nom_village 
from resserre r
...
select v.nom_village, count (h.num_village)as nb_pax from habitant h 
...
select h.nom, count (distinct p.num_potion) as nb_diff_pot  from potion p
join absorber a on p.num_potion = a.num_potion 
join habitant h on a.num_hab  = h.num_hab 
group by h.nom ; 
--27. Nom des habitants ayant bu plus de 2 louches de potion zen. (1 ligne)
select h.nom, a.quantite  from absorber a 
join potion p on a.num_potion = p.num_potion 
join habitant h on a.num_hab = h.num_hab 
...
join resserre r2 on v.num_village = r2.num_village 
where exists ( select r.nom_resserre from resserre r);
...
--mdonne pas le bon resultat


--select h.nom, count (t.num_preneur) from trophee t
--join habitant h  on t.num_preneur  = h.num_hab 
--where t.num_preneur > (select count (t2.num_preneur) from trophee t2)  and (h.nom > 'Obélix') 
--group by h.nom;
...
--select h.nom, count (t.num_preneur) from trophee t
--join habitant h  on t.num_preneur  = h.num_hab 
--where t.num_preneur > (select count (t2.num_preneur) from trophee t2)
--group by h.nom;

--select h.nom from
--join habitant h on t.num_preneur = h.num_hab 
--where = (select count(t.num_preneur)from trophee t) > (select count(t2.num_preneur) from trophee t2) and h.nom = "Obélix";
